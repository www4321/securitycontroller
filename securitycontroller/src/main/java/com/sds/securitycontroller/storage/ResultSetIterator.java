 ...
 */
public class ResultSetIterator implements Iterator<IStorageResultSet> {
    private IStorageResultSet resultSet;
    private boolean hasAnother;
    private boolean peekedAtNext;
    
    public ResultSetIterator(IStorageResultSet resultSet) {
        this.resultSet = resultSet;
        this.peekedAtNext = false;
    }
    
    @Override
    public IStorageResultSet next() {
        if (!peekedAtNext) {
